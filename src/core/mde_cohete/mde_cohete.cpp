...
namespace Cohete {
    ...
    const Cohete::f_st_t MDE_COHETE[] = {
        f_st_init,
    ...
    void mde_cohete_actualizar(data_all_t* datos_sensores) {
        if (SYSTEM.estado > Cohete::ST_NULL) {
            transicion_error(Cohete::ERR_ESTADO_INVALIDO, datos_sensores);
        }
    ...
    void transicion_error(const Cohete::cod_error_t error, data_all_t *datos_sensores) {
        SYSTEM.error=error;
        transicionar_hacia(Cohete::ST_ERROR);
    ...
    void transicionar_hacia(Cohete::estado_t nuevo_estado) {
        if (nuevo_estado > Cohete::ST_NULL) {
